...
| P1-49 | **[V-E2E-01] AIOps no workspace = tela em branco** — `/clusters/{id}/aiops` renderiza sem conteúdo. Nenhum empty state, spinner ou erro visível. AIOps global (`/aiops`) funciona. Causa provável: componente `ClusterAIOpsPage` não tem guard para dados vazios ou a rota não está passando o clusterId correto ao componente. Fix: verificar o componente que renderiza a rota de cluster AIOps e adicionar guard + empty state. | ❌ Detectado E2E 2026-05-26 |
| P1-50 | **[V-E2E-02] Security "Fix" button = dead click** — botões "Fix" nas TOP FINDINGS da Security Intelligence não executam nenhuma ação. Sem modal, painel ou navegação. Fix: implementar ação (modal de detalhes do finding com sugestão de remediação, ou navegar para o workload afetado). | ❌ Detectado E2E 2026-05-26 |
| P1-51 | **[V-E2E-03] Security TOP FINDINGS sem descrição do finding** — cada item exibe apenas "Finding" sem título/descrição real. A API retorna o campo vazio ou o frontend não está mapeando o campo correto (`title`, `description`, `message`). Fix: mapear campo de descrição da API e renderizar no card. | ❌ Detectado E2E 2026-05-26 |
| P1-52 | **[V-E2E-04] Billing migração 000005 não incluída no runMigrations** — tabelas `ai_credit_packs`, `org_ai_credits`, `ai_credit_transactions`, `stripe_events` não são criadas automaticamente na inicialização do billing service. `GET /billing/v1/ai/packs` retorna 500. Fix: adicionar `"000005_ai_credits.up.sql"` à lista em `billing/cmd/server/main.go:runMigrations()`. | ❌ Bug crítico detectado E2E 2026-05-26 |
| P1-53 | **[V-E2E-05] Observability cluster drill-down: seção Namespaces vazia** — ao entrar no cluster dev-local na tela Observability, a seção "NAMESPACES — DEV-LOCAL" carrega sem cards. Provavelmente a query de namespaces no contexto Observability não está sendo disparada ou está falhando silenciosamente. | ❌ Detectado E2E 2026-05-26 |
| P1-54 | **[V-E2E-06] "Generate fix PR" na Security = comportamento não implementado** — botão visível no card AI Summary da Security mas sem ação ao clicar. Remover ou implementar (ex: abrir modal com diff YAML sugerido + botão de copy/apply). | ❌ Detectado E2E 2026-05-26 |
| P1-55 | **[V-E2E-07] "Configure provider →" no Navy panel não ativa aba AI Providers** — o botão navega para `/settings` mas aterra na aba Organization em vez de AI Providers. Fix: usar `navigate(\`/${orgId}/settings?tab=AI+Providers\`)` ou equivalente para pre-selecionar a aba correta. | ❌ Detectado E2E 2026-05-26 |
| P1-56 | **[V-E2E-08] Score de risco na Security muda confusamente ao trocar aba** — o scorecard 90/100 "Elevated Risk" muda para 0/100 "Low Risk" ao ir de Config Risk para RBAC Risk. Não há contexto que o score é por categoria. Fix: exibir score geral fixo + score contextual da aba ativa como dois valores distintos, ou adicionar label "Score desta categoria". | ❌ Detectado E2E 2026-05-26 |
| P1-57 | **[V-E2E-09] intelligence/summary com latência de 6.4 segundos** — endpoint mais lento do produto, bloqueia carregamento da página inicial. Cache server-side com TTL 45s já existe em `intelligence_cache.go` mas pode não estar sendo ativado corretamente para org sem sinais. Investigar e garantir que o cache é hit no segundo request. | ❌ Detectado E2E 2026-05-26 |
| P1-58 | **[V-E2E-10] Topology: sidebar colapsado exibe ícones sem tooltip** — com dois sidebars abertos, o sidebar esquerdo fica em modo ícone-only sem tooltip no hover. Usuário não sabe o que cada ícone representa. Fix: adicionar tooltip `title` ou `aria-label` em cada link do sidebar colapsado. | ❌ Detectado E2E 2026-05-26 |
