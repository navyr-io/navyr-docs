...
**Última atualização:** 2026-05-27 (P1-55..58 — bugs E2E restantes corrigidos)  
...
| P1-49 | **[V-E2E-01] AIOps no workspace = tela em branco** — falso positivo: o audit navegou para `/clusters/{id}/aiops` (rota inexistente). A aba "AI Ops" no WorkspaceTabBar aponta para `/ai-operations` e a página renderiza corretamente. Nenhuma correção necessária. | ✅ Investigado e fechado 2026-05-26 |
| P1-50 | **[V-E2E-02] Security "Fix" button = dead click** — botão "Fix" no TOP FINDINGS agora navega para o workload afetado (`/clusters/{id}/workloads/pods?namespace=...`). Visível apenas quando o finding tem workload mapeável. | ✅ Corrigido 2026-05-26 |
| P1-51 | **[V-E2E-03] Security TOP FINDINGS sem descrição do finding** — `configRiskFinding` da API não tem `title/reason/severity` no nível raiz (estão em `checks[]`). Adicionados helpers `findingSeverity()`, `findingTitle()`, `findingDesc()` para normalizar ambas as shapes (configRisk e rbacRisk). TOP FINDINGS agora exibe título e descrição reais. | ✅ Corrigido 2026-05-26 |
| P1-52 | **[V-E2E-04] Billing migração 000005 não incluída no runMigrations** — `"000005_ai_credits.up.sql"` adicionado ao `runMigrations()` em `billing/cmd/server/main.go`. Index criado com `IF NOT EXISTS` para idempotência. Billing agora inicia com PostgreSQL. | ✅ Corrigido 2026-05-26 |
| P1-53 | **[V-E2E-05] Observability cluster drill-down: seção Namespaces vazia** — `requestJson` chamado sem `clusterId`, então o header `X-Cluster-ID` estava ausente. Backend rejeita com 400. Adicionado `clusterId` às queries `namespacesQ` e `workloadsQ`. | ✅ Corrigido 2026-05-26 |
| P1-54 | **[V-E2E-06] "Generate fix PR" na Security = comportamento não implementado** — botão removido. "View attack path" agora navega para a aba attack-path inline. | ✅ Corrigido 2026-05-26 |
| P1-55 | **[V-E2E-07] "Configure provider →" no Navy panel não ativa aba AI Providers** — `SettingsPage` agora lê `?tab=` via `useSearchParams` para inicializar a aba ativa. `AIAssistant` navega para `?tab=AI+Providers`. | ✅ Corrigido 2026-05-27 |
| P1-56 | **[V-E2E-08] Score de risco na Security muda confusamente ao trocar aba** — `ScoreRing` agora exibe label "{categoria} score" (ex: "Config Risk score") acima do riskLabel. Aceita `null` para abas sem score numérico (RBAC), exibindo "—" em vez de 0/100. | ✅ Corrigido 2026-05-27 |
| P1-57 | **[V-E2E-09] intelligence/summary com latência de 6.4 segundos** — cache backend confirmado funcional (TTL=45s). Latência é de cold MISS (primeira carga ou após expiração). Frontend: `staleTime` aumentado 30→60s para evitar re-request no janela 30-45s; `placeholderData` mantém dados anteriores visíveis durante re-fetch; indicador de pulse sutil adicionado. | ✅ Mitigado 2026-05-27 — latência inicial é backend (Codex P2) |
| P1-58 | **[V-E2E-10] Topology: sidebar colapsado exibe ícones sem tooltip** — `GlobalSidebar` agora renderiza tooltip `data-tip` via `position: absolute` (aparece instantaneamente no hover, sem delay de 500ms do `title` nativo). Adicionado `aria-label` para acessibilidade. | ✅ Corrigido 2026-05-27 |
| P1-59 | **Admin > Groups — painel GroupDetail com catálogo IAM granular completo** — `AdminPage.tsx` `GroupDetail` redesenhado: layout 3 abas (IAM Permissions \| Members \| K8s RBAC); constante `IAM_CATALOG` com todos os recursos (`clusters`, `workloads`, `security`, `observability`, `approvals`, `automation`, `settings`, `aiops`, `finops`, `intelligence`, `labs`) × ações granulares; tipo `GroupPermission`; toggle por permissão chamando `POST /auth/admin/groups/{id}/permissions` e `DELETE /auth/admin/groups/{id}/permissions/{pid}`; badge de contagem de permissões ativas no cabeçalho; `key={selected.id}` força remount ao trocar de grupo. | ✅ Entregue 2026-05-26 — commit 88a437a; pushed navyr-frontend 7f9f26b |
...
| SEC-04 | **[HIGH] SSRF via endpoint de AI provider configurado pelo admin** — `gateway/main.go:~1609`: campo `endpoint` do provider (armazenado no DB, configurado pelo admin) é usado como destino HTTP direto sem allowlist. Admin comprometido → SSRF para metadata cloud (169.254.169.254) ou serviços internos | High | Codex (gateway) | ❌ Detectado análise de segurança 2026-05-26 |
| SEC-05 | **[HIGH] SSRF via URLs de webhooks registrados por usuário** — `auth/main.go:493,1059`: URL do webhook validada apenas como URI válida (não restringe IPs privados). Qualquer usuário autenticado pode apontar webhook para serviços internos. Fix: validar que é HTTPS + hostname público + bloquear ranges privados | High | Codex (auth) | ❌ Detectado análise de segurança 2026-05-26 |
| SEC-06 | **[HIGH] Webhook delivery envia secret em plaintext no header** — `auth/main.go:1064`: `X-Webhook-Secret: <raw_secret>`. Fix: assinar payload com HMAC-SHA256, enviar `X-Webhook-Signature: sha256=<hex>`. Não enviar o secret diretamente | High | Codex (auth) | ❌ Detectado análise de segurança 2026-05-26 |
| SEC-07 | **[HIGH] Stripe checkout aceita open redirect via success/cancel URL** — `billing/main.go:638`: `success_url` e `cancel_url` do corpo do request repassados verbatim ao Stripe sem validação de domínio. Fix: validar que URLs começam com `FRONTEND_URL` configurado | High | Codex (billing) | ❌ Detectado análise de segurança 2026-05-26 |
| SEC-08 | **[MEDIUM] Stripe params construídos com fmt.Sprintf — injection risk** — `billing/main.go:638`: parâmetros do Stripe montados via string concatenation. Fix: usar `url.Values{}` e `.Encode()` | Medium | Codex (billing) | ❌ Detectado análise de segurança 2026-05-26 |
| SEC-09 | **[MEDIUM] WebSocket /exec/ws sem check de credencial no gateway** — `gateway/main.go:359`: rota proxied para orchestrator sem verificar presença de ticket. Fix: verificar `X-WS-Ticket` header não-vazio antes de proxy | Medium | Codex (gateway) | ❌ Detectado análise de segurança 2026-05-26 |
| SEC-10 | **[MEDIUM] clusterID não escapado em URL de autorização interna** — `gateway/main.go:2820`: `"/internal/v1/clusters/" + clusterID + "/authorize"` sem `url.PathEscape`. Fix: `url.PathEscape(clusterID)` como já feito em outros paths do billing | Medium | Codex (gateway) | ❌ Detectado análise de segurança 2026-05-26 |
| SEC-11 | **[MEDIUM] /metrics sem auth no billing e orchestrator** — diferente do gateway (que tem `protectMetricsHandler`), billing e orchestrator expõem `/metrics` sem proteção. Fix: aplicar mesmo padrão do gateway | Medium | Codex (billing+orchestrator) | ❌ Detectado análise de segurança 2026-05-26 |
| SEC-12 | **[MEDIUM] X-Forwarded-For confiado diretamente — IP spoofável** — `gateway/main.go:2771`: clientIP usa primeiro valor do XFF sem verificar proxy confiável. Bypassável para rate limiting e log de auditoria | Medium | Codex (gateway) | ❌ Detectado análise de segurança 2026-05-26 |
| SEC-13 | **[LOW] JWT e refresh tokens em localStorage** — vulneráveis a XSS. Migrar para HttpOnly cookies seria mais seguro | Low | Claude (frontend) | ❌ Detectado análise de segurança 2026-05-26 |
| SEC-14 | **[LOW] Endpoints internos /dispatch e /approval sem autenticação** — `auth/main.go:648,672`: qualquer pod no cluster pode disparar webhook events ou notificações de approval. Fix: exigir assinatura interna (mesmo mecanismo gateway→orchestrator) | Low | Codex (auth) | ❌ Detectado análise de segurança 2026-05-26 |
...
- **SEC-04..14:** Ver backlog acima — prioridade recomendada: SEC-04, SEC-05, SEC-06, SEC-07 primeiro (todos HIGH).
