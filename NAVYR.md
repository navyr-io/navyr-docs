...
| SEC-04 | **[HIGH] SSRF via endpoint de AI provider configurado pelo admin** — `gateway/main.go:~1609`: campo `endpoint` do provider (armazenado no DB, configurado pelo admin) é usado como destino HTTP direto sem allowlist. Admin comprometido → SSRF para metadata cloud (169.254.169.254) ou serviços internos | High | Codex (gateway) | ✅ Corrigido 2026-08-18 (Fase 1) |
| SEC-05 | **[HIGH] SSRF via URLs de webhooks registrados por usuário** — `auth/main.go:493,1059`: URL do webhook validada apenas como URI válida (não restringe IPs privados). Qualquer usuário autenticado pode apontar webhook para serviços internos. Fix: validar que é HTTPS + hostname público + bloquear ranges privados | High | Codex (auth) | ✅ Corrigido 2026-08-18 (Fase 1) |
| SEC-06 | **[HIGH] Webhook delivery envia secret em plaintext no header** — `auth/main.go:1064`: `X-Webhook-Secret: <raw_secret>`. Fix: assinar payload com HMAC-SHA256, enviar `X-Webhook-Signature: sha256=<hex>`. Não enviar o secret diretamente | High | Codex (auth) | ✅ Corrigido 2026-08-18 (Fase 1) |
| SEC-07 | **[HIGH] Stripe checkout aceita open redirect via success/cancel URL** — `billing/main.go:638`: `success_url` e `cancel_url` do corpo do request repassados verbatim ao Stripe sem validação de domínio. Fix: validar que URLs começam com `FRONTEND_URL` configurado | High | Codex (billing) | ✅ Corrigido 2026-08-18 (Fase 1) |
| SEC-08 | **[MEDIUM] Stripe params construídos com fmt.Sprintf — injection risk** — `billing/main.go:638`: parâmetros do Stripe montados via string concatenation. Fix: usar `url.Values{}` e `.Encode()` | Medium | Codex (billing) | ✅ Corrigido 2026-08-18 (Fase 1) |
| SEC-09 | **[MEDIUM] WebSocket /exec/ws sem check de credencial no gateway** — `gateway/main.go:359`: rota proxied para orchestrator sem verificar presença de ticket. Fix: verificar `X-WS-Ticket` header não-vazio antes de proxy | Medium | Codex (gateway) | ✅ Corrigido 2026-08-18 (Fase 1) |
| SEC-10 | **[MEDIUM] clusterID não escapado em URL de autorização interna** — `gateway/main.go:2820`: `"/internal/v1/clusters/" + clusterID + "/authorize"` sem `url.PathEscape`. Fix: `url.PathEscape(clusterID)` como já feito em outros paths do billing | Medium | Codex (gateway) | ✅ Corrigido 2026-08-18 (Fase 1) |
| SEC-11 | **[MEDIUM] /metrics sem auth no billing e orchestrator** — diferente do gateway (que tem `protectMetricsHandler`), billing e orchestrator expõem `/metrics` sem proteção. Fix: aplicar mesmo padrão do gateway | Medium | Codex (billing+orchestrator) | ✅ Corrigido 2026-08-18 (Fase 1) |
| SEC-12 | **[MEDIUM] X-Forwarded-For confiado diretamente — IP spoofável** — `gateway/main.go:2771`: clientIP usa primeiro valor do XFF sem verificar proxy confiável. Bypassável para rate limiting e log de auditoria | Medium | Codex (gateway) | ✅ Corrigido 2026-08-18 (Fase 1) |
| SEC-13 | **[LOW] JWT e refresh tokens em localStorage** — vulneráveis a XSS. Migrar para HttpOnly cookies seria mais seguro | Low | Claude (frontend) | ✅ Corrigido 2026-08-18 (Fase 1) |
| SEC-14 | **[LOW] Endpoints internos /dispatch e /approval sem autenticação** — `auth/main.go:648,672`: qualquer pod no cluster pode disparar webhook events ou notificações de approval. Fix: exigir assinatura interna (mesmo mecanismo gateway→orchestrator) | Low | Codex (auth) | ✅ Corrigido 2026-08-18 (Fase 1) |
